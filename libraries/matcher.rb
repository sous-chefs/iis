if defined?(ChefSpec)
  ...
  [:config, :add, :delete].each do |action|
    self.class.send(:define_method, "#{action}_iis_root", proc do |root|
        ChefSpec::Matchers::ResourceMatcher.new(:iis_app, action, root)
      end
    )
  end
  ...
  [:add, :config, :delete].each do |action|
    self.class.send(:define_method, "#{action}_iis_vdir", proc do |vdir|
        ChefSpec::Matchers::ResourceMatcher.new(:iis_vdir, action, vdir)
      end
    )
  end
end
