...
action :add do
  unless @current_resource.exists
    ...
    if !verify_creation_occurred?(method(:pool_exists?))
      Chef::Log.error("Application Pool wasn't created within 5 seconds...")  
    else
      configure
    end
...
private
  ...
  def pool_exists?
    ...
    if cmd.stderr.empty?
      ...
      result = result.match(/^APPPOOL\s\"(#{new_resource.pool_name})\"/)
      Chef::Log.debug("#{new_resource} current_resource match output: #{result}")
      if result
        exists = true
      else
