...
directory "#{docroot}\\vdir_test" do
  recursive true
end

directory "#{docroot}\\foo" do
  recursive true
end

directory "#{docroot}\\app_test" do
  recursive true
end

directory "#{docroot}\\app_test\\vdir_test2" do
  recursive true
end
...
iis_app 'Default Web Site' do
  ...
  physical_path "#{docroot}\\app_test"
...
iis_vdir 'Default Web Site/' do
  path '/vdir_test'
  physical_path "#{docroot}\\vdir_test"
  username 'vagrant'
  password 'vagrant'
...
iis_vdir 'Creating vDir /vdir_test2 in app' do
  ...
  path '/vdir_test2'
  physical_path "#{docroot}\\app_test\\vdir_test2"
  action [:add, :config]
end
